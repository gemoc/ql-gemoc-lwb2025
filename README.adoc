...
As seen in Section <<section-quick-start>>, in addition to allow edition which are actually provided by embedded tools such as Xtext and Sirius, GEMOC Studio has a special focus on providing model debugging support. 

This section provides details of some of the features used by the Questionnaire language.

By default in GEMOC, launching a model is done via a launch configuration. Figure <<img-mwb-projectBudget-launch-maintab>> show the main tab of launch configuration. It allows to select the model to run, the language semantic that must be used to execute the language (as we may have provided several variant semantics), the entry point (ie. the model element used to start the execution. this is useful as some DSL may contains several starting points), and the main method (I.e, the `@Main` method, it may be used in some DSL implementation that provides different main methods).

Additionally, we can specify arguments (a list of strings) to pass to the `@InitializeModel` model. So it can intialize any relevant runitme data prior to start the execution.

We can also specify a Sirius graphical representation. When started, it will automatically activate the `Animation` layer as it is declared in the `AbstractGemocAnimatorServices` class that extends `AbstractGemocAnimatorServices`.

[TIP]
====

As GEMOC Studio is an Eclipse IDE, it is possible to provide UI in order to start action in many other ways: wizards, Menu,  popup,... 
====

.Main tab of the launch configuration for the ProjectBudgetEstimation questionnaire example model)
[#img-mwb-projectBudget-launch-maintab]
image::mwb_projectBudget_launch_main_tab.png[Main tab of the launch configuration for the ProjectBudgetEstimation questionnaire example model,1024]

Figure <<img-mwb-projectBudget-launch-addon-tab>> shows how to activate/desactivate the optional Engine addons.
The _Generic Multidimentional Data Trace_ enable the storage of runtime data so it can be used by the _MultiDimentional Data timeline_. This addon and its view are generic and available for any DSL using the sequential engine.

The _QLForm Browser View_ addon provide the UI for the language. It is specific to the QL DSL. You need to enable it in order to display the challenge Form UI. (As stated in section <<section-behavioral-semantics>>, if other addon provide alternative UI, you could have made the choice here)


.Engine addon tab of the launch configuration for the ProjectBudgetEstimation questionnaire example model)
[#img-mwb-projectBudget-launch-addon-tab]
image::mwb_projectBudget_launch_addon_tab.png[Engine addon tab of the launch configuration for the ProjectBudgetEstimation questionnaire example model,1024]

When started in Debug mode, the Eclipse IDE will provide several views.
If the animation is active, the Sirius editor will be closed, and will reopen in readonly mode.

Figure <<img-mwb-projectBudget-debug-timeline-paused-after-enabling-designUX-checkbox>> shows several noticiable elements:

* The _QLForm Browser View_ allows user interaction with the form, displaying questions, results, and a Submit button to end the execution when the user has finished.
* The _Sirius view_ displays with a small blue bullet that a break point is active and will pause the execution whenever a `@Step` method is called on the  `totalProjectCost` `QuestionDefinition`. 
* When paused due to a breakpoint, all classical, step into, step over button from eclipse are available in order to continue the execution to the desired following `@Step`.
* When paused as in the figure, the stack view on the left allows to select the current @Step (or the @Step that is going to be called). This selection automatically find the related model element in the Xtext editor, and also highlight it in yellow in the _Sirius view_. (thanks to the Debug layer)
* The _Variable view_ allows to navigate into the runtime data of the running model.
* The `Animation layer` in the Sirius View shows some runtime data: the current value of questions, indicates with a color if a question is displayed or not (grey if not displayed), status of the Submit button (as it we cannot submit the form, if some mandatory question isn't answered), ...
* The _Multidimentional Timeline_ view shows the runtime data changes. 
** Each blue bullet indicates that some data has changed. It isn't strictly connected to the @Step in the sense of may be several @Step has been called without any data change. @Step call stack is represented with the lines bellow the blue bullets. 
** Each runtime data has its own timeline. They provide a line that is interrupted when the value changes.

for example in Figure <<img-mwb_projectBudget_debug_timeline_paused_after_enabling_designUX_checkbox>>, the execution is paused when the user has checked the `requireDesignAndUX` checkBox, so even if she has entered values for the preceding value, has it has enabled new question, the submit button must now be disabled. The timeline shows the changes for each individual data. 

TIP: It is possible to kill an execution at all time by using the _GEMOC Engine Status_ view.
...
image::mwb_projectBudget_debug_timeline_paused_after_enabling_designUX_checkbox.png[Debug of a ProjectBudget example model paused after enabling the requireDesignAndUX question (with MultiDimentional data timeline enabled),1024]

Figure <<img-mwb-projectBudget-debug-timeline-with-variables>> provide a better view of the display provided by the _Variable view_. It is paused when the user has changed the `estimatedPersonHourForDev` value.

.Debug of a ProjectBudget example model paused after enabling changing the estimatedPersonHourForDev value (with MultiDimentional data timeline enabled)
[#img-mwb-projectBudget-debug-timeline-with-variables]
image::mwb_projectBudget_debug_timeline_with_variables.png[Debug of a ProjectBudget example model paused after enabling changing the estimatedPersonHourForDev value,1024]

It it then possible to *navigate backward and forward in the timeline*. Either by clicking on the blue bullets, or in a data line in order to follow this data changes.

Figure <<img-mwb-projectBudget-debug-timeline-with-backward-navigation>> illustrates this backward navigation.
It is then  possible to look into the _Variables view_ or the _Sirius Animation view_ to inspect the status of the model at that time. We can observe for example, that the `estimatedPersonHourForDev` value is now back to `7` and the `estimatedDevCost` to `140`.

[NOTE]
====

Some UI may also be able to refresh according to the current status of the model at this point in time. However, this highly dependent on the technology used build the UI.c
====
[NOTE]
====

Unlike the concurrent engine presented in Section <<section-concurrency-mmodeling>>, the backward navigation proposed by the sequential engine used by K3QL isn't a full rollback of the engine. 
It cannot restart the execution from an older state and simply offers to navigate in the recorded trace. 

However, it restores the model to the previous state, so the animation and compatible addons can use this information to display the model at that point in time.

On that point, the GEMOC concurrent engine is more powerful than the GEMOC sequential engine as it can branch execution from a state in the past.

====

.Debug of a ProjectBudget example model paused after enabling changing the estimatedPersonHourForDev value but using backward navigation to display a previous state.e[#img-mwb-projectBudget-debug-timeline-with-backward-navigation]
image::mwb_projectBudget_debug_timeline_with_backward_navigation.png[Debug of a ProjectBudget example model paused after enabling changing the estimatedPersonHourForDev value but using backward navigation to display a previous state,1024]
