...
Some UI, may be able to also refresh according to the current status of the model at this point in time. However, this highly depend on the technology used build the UI.c
====
[NOTE

.Debug of a ProjectBudget example model paused after enabling changing the estimatedPersonHourForDev value but using backward navigation to display a previous state
[#img-mwb_projectBudget_debug_timeline_with_backward_navigation]
