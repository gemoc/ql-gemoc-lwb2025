...
As seen in Section <<section-quick-start>>, in addition to allow edition whihc are actually provided by embedded tools such as Xtext and Sirius, GEMOC Studio has a special focus on providing model debugging support.

This section provides some details of the proposed features implemented for the Questionnaire language.

By default in GEMOC, launching a model is done via a launch configuration.Figure <<img-mwb_projectBudget_launch_main_tab>> show the main tab of launch configuration. It allows to select the model to run, the language semantic that must be used to execute the language (as we may have provided several variant semantics), the entry point (ie. the model element used to start the execution. this is useful as some DSL may contains several starting points), and the main method (I.e, the `@Main` method, it may be used in some DSL implementation that provides different main methods).
...
[#img-mwb_projectBudget_launch_main_tab]
...
Figure <<img-mwb_projectBudget_launch_addon_tab>> shows how to activate/desactivate the optional Engine addons.
...
[#img-mwb_projectBudget_launch_addon_tab]
...
Figure <<img-mwb_projectBudget_debug_timeline_paused_after_enabling_designUX_checkbox>> shows several noticiable elements:
...
Figure <<img-mwb_projectBudget_debug_timeline_with_variables>> provide a better view of the display provided by the _Variable view_. It is paused when the user has changed the `estimatedPersonHourForDev` value.
...
[#img-mwb_projectBudget_debug_timeline_with_variables]
...
It it then possible to navigate backward in the timeline. Either by clicking on the blue bullets, or in a data line in order to follow this data changes.

Figure <<img-mwb_projectBudget_debug_timeline_with_backward_navigation>> illustrates this backward navigation.
It is the  possible to look into the variable view or the Sirius Animation view to inspect the status of the model at that time.
...
.Debug of a ProjectBudget example model paused after enabling changing the estimatedPersonHourForDev value but using backward navigation to display a previous state.e[#img-mwb_projectBudget_debug_timeline_with_backward_navigation]]
