...
In the engine, the execution of the addon methods is atomic. The engine ensures that all defined addon methods have been completly executed before continuing to the next step.
As it pauses the model execution and provide access to runtime data, this is handy to implement the `QLModel.waitUserInput()` step.
