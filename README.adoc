...
[NOTE]
====
Several preconfigured launch configuration are provided, they illustrates various options such as:
...
[IMPORTANT]
====
In order to have a functionnal launch configuration, the `org.gemoc.ql.k3based.addons.views.QLFormBrowserView` in the addon tab must be activated. Otherwise the execution will take no input from the user (see discussion about this implementation later in this document)
====
...
image::mwb_houseOwning_run_without_qls.png[Execution of a quesionnaire without QLS]

.Execution of a questionnaire with QLS
[#img-mwb-houseowning-without-qls]
image::mwb_houseOwning_run_with_qls.png[Execution of a quesionnaire with QLS]
...
A `QLSModel`` decorates a `QLModel`. It specifies `Section`s that contains ordered content (other sub-sections or references to `QuestionDefinition`). These information are to organise the order and grouping of questions on the view. The actual visibility of a question remains a responsability of the QL model.

`QuestionStyle` decorates a `QuestionDefinition` to provide some visual customisation on the label or on the question type.


NOTE: The `Import` coming from xText allows to have simple and precise reference to imported models. It is used to reference the resource containing the QL model with textual syntax.
...
The code for the Type checking used by Xtext editor, uses the same approach as the execution semantics: it uses K3 aspects. Xtext validation framework triggers the validation in the editor and then call the aspects for inferring the types and checking compatibility.
uThe approach allows to reuse the same code in other validation frameworks such as the EMF validation framework or the Sirius internal validation. This can be handy in case we allow edition of the model using other editors than the textual one.
